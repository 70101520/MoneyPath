...
This records the Phase 1 baseline. Phase 2 is now implemented and deployed; current results, model assumptions and later-phase boundaries are in [PHASE2.md](PHASE2.md).
...
| Docker image build and Compose startup              | Passed on Ubuntu testing VM: image build, migrations, Compose startup and non-root UID 1001              |
...
| Container smoke-test script against standalone app  | Passed on 14-09-2026 against actual Docker containers with a fresh isolated database                     |
...
The browser pass does not constitute a comprehensive accessibility audit or Safari/Firefox/device certification. Docker verification passed on the Ubuntu testing VM, including all 29 tests inside the build image and a PostgreSQL 17 dump/restore comparison of all 14 tables with decrypted private notes. `.github/workflows/phase1.yml` now builds and starts a disposable Compose stack, tests real authentication and accounting endpoints, checks its non-root runtime, and runs finance/security tests inside the build image. Equivalent checks passed on the VM; a GitHub-hosted workflow result has not been confirmed.
...
The deployed VM at http://192.168.80.128:3000 also passed LAN Chromium checks for login, protected-route redirects, anonymous API rejection, and desktop/mobile sample rendering without page errors or horizontal overflow. Screenshots: artifacts/vm-desktop.png and artifacts/vm-mobile.png. See [DEPLOYMENT.md](DEPLOYMENT.md) for operations and the tested application revision.
