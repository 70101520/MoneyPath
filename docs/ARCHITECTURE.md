...
Current status: Phases 1–4 implemented. Phase 4 includes a constrained finance assistant, expiring hashed-token read-only mobile API, reviewed CSV import, and deduplicated SMTP/Web Push notification delivery. External delivery activates only after provider credentials are configured. See [PHASE3.md](PHASE3.md) and [PHASE4.md](PHASE4.md). Budget, salary-plan, debt-plan and priority-cost writes use the authenticated, audited, idempotent records API. Salary acceptance also validates a source-data fingerprint in a serializable transaction. Budget limits are monthly tracking targets, not additional cash reservations; missing budgets remain unknown. Risk snapshots retain versioned rules and, for new snapshots, tracked cash/debt/assets/net worth.
