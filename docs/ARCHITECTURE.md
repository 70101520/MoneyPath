...
Current status: Phases 1, 2 and 3 implemented. Phase 4 has a constrained finance assistant and expiring, hashed-token read-only mobile API; external provider activation remains pending. See [PHASE3.md](PHASE3.md) and [PHASE4.md](PHASE4.md). Budget, salary-plan, debt-plan and priority-cost writes use the authenticated, audited, idempotent records API. Salary acceptance also validates a source-data fingerprint in a serializable transaction. Budget limits are monthly tracking targets, not additional cash reservations; missing budgets remain unknown. Risk snapshots retain versioned rules and, for new snapshots, tracked cash/debt/assets/net worth.
